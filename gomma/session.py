...
class Session(object):
    ...
    def __manageRequestResponse(self, r: requests.Response):
        ...
        logging.debug(f"manage requests response {r.url} ({r.elapsed}) {r.status_code}")
        response = {"status": r.ok == True}
        if not r.text:
            return response
    ...
    def __setGomma(self, uid: str = None):
        ...
        try:
            ...
            # sottraggo 1 minuti: meglio che scada prima ag:gomma che la authkey delle api!
            tokenExpireAt = datetime.fromtimestamp(expires_at) - timedelta(minutes=1)
        except:
            logging.error("Unable to read expires_at!. Set default value 15min")
            tokenExpireAt = datetime.now() + timedelta(minutes=15)

        uid = apitoken["access_token"]
        data = {"uid": uid} 
        self.redis.hmset(self.__cacheKey, data)
        self.redis.expireat(self.__cacheKey, tokenExpireAt)
        return True
    ...
    def __getGomma(self):
        ...
        if self.redis.exists(self.__cacheKey):
            ...
            if gommattl < 5:
                ...
                logging.warning("gomma almost expired! ({}s left)".format(gommattl))
                # refresh session
                uid = self.redis.hmget(self.__cacheKey, "uid")[0]
                self.__setGomma(uid)
        else:
